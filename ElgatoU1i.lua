...
function redzlib:MakeWindow(Configs)
    ...
    local Rounding = Configs.Rounding or 1  -- Default rounding to 1 if not provided
    ...
    local function UpdateLabel(NewValue)
        -- Apply rounding based on the Rounding value
        local RoundedValue = math.floor(NewValue / Rounding + 0.5) * Rounding  -- Round the value
        Default, LabelVal.Text = RoundedValue, tostring(RoundedValue)
        Funcs:FireCallback(Callback, Default)
    end
